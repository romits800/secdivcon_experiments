...
flags="$flags --global-budget 50 --local-limit 50000"
#importfl="--copyblck" 
#flags="$flags --sec-implementation sec_reg_2_mem_2"
timeout=5400000

$UNI import --target=$target ${aflags} $name.mir -o $name.uni --function=$func $importfl --goal=speed --maxblocksize=$bsize
...
# Solving
$SOLV/gecode-solver $flags -o $name.$iter.out.json --verbose $name.ext.json
